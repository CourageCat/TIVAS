                ...
                if (!ticketDuplicated) {
                  ...
                  if (!userUsedTicket) {
                    ...
                    const reservationTicket =
                      await db.ReservationTicket.findOne({
                        where: {
                          code,
                        },
                      });
            ...
            if (ticketResponse.length !== 0) {
              ...
              for (let i = 0; i < count1; i++) {
                ...
                for (let x = 0; x < quantityTimeshare.quantity; x++) {
                  const reservation =
                    result[Object.getOwnPropertyNames(result)[i]][x];
                  if (reservation) {
                    await db.ReservationTicket.update(
                      {
                        status: 2,
                    ...
                    await db.Booking.create({
                      ...
                      status: 0,
                      priceBooking:
                        timeShare.price - reservation.reservationPrice,
                    ...
                    const emailTemplatePath =
                      "src/template/EmailWinner/index.ejs";
...
export const getAllUserPriorityByAdmin = (
  id,
  { page, limit, orderBy, orderType }
) => {
  return new Promise(async (resolve, reject) => {
...
export const getAllUserNoPriorityByStaff = ({
  id,
  userID,
  page,
  limit,
                ...
                for (let i = 0; i < ticketResponse.length; i++) {
                  ...
                  if (parseInt(status) !== 1) {
                    ...
                    if (parseInt(status) === 2 || parseInt(status) === 4) {
                      ...
                      ticket.bookingTimeShareDate =
                        ticketResponse[i].bookingDate;
                    } else {
                      ticket.bookingStatus = ticketResponse[i].Booking.status;
                  ...
                  if (
                    !(
                      ticketResponse[i].Project.status === 3 &&
                      (+status === 2 || +status === 1)
                    ) &&
                    !(
                      ticketResponse[i].Project.status !== 3 && +status === 4
                    ) &&
                    !(
                      ticketResponse[i].refund === 1 &&
                      (+status === 2 || +status === 1)
                    )
                  ) {
                    console.log(123);
